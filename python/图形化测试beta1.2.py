...
def read_and_merge_files(file_names):
    ...
    for file_name in file_names:
        file_path = fr'C:\Users\{current_username}\Desktop\{file_name}'
        if not os.path.exists(file_path):
            messagebox.showerror("错误", f"文件 {file_name} 不存在，请检查文件名和路径。")
            return
...
def on_merge():
    ...
    if not all(file_entries):
        ...
        return
    file_names = [os.path.basename(entry) for entry in file_entries]  # 获取完整文件名

...
# 输出DataFrame的头部以检查BMI列
print("计算后的BMI值：")
print(df.head())

# 保存包含BMI的Excel文件
intermediate_output_file_path = 'C:\\Users\\andre\\Desktop\\excel\\学校\\测试用表\\1.1.xlsx'
df.to_excel(intermediate_output_file_path, index=False)

print(f'包含BMI的Excel文件已保存：{intermediate_output_file_path}')

import pandas as pd

# 读取包含BMI的Excel文件
intermediate_file_path = 'C:\\Users\\andre\\Desktop\\excel\\学校\\测试用表\\1.1.xlsx'
df = pd.read_excel(intermediate_file_path)
...
# 输出DataFrame的头部以检查BMI评分列
print("计算后的BMI评分：")
print(df.head())

# 保存包含BMI评分的Excel文件
final_output_file_path = 'C:\\Users\\andre\\Desktop\\excel\\学校\\测试用表\\1.2.xlsx'
df.to_excel(final_output_file_path, index=False)

print(f'包含BMI评分的Excel文件已保存：{final_output_file_path}')